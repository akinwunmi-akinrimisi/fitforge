...
export default function LoginPage({ searchParams }: { searchParams: { next?: string } }) {
  const next =
    searchParams.next && searchParams.next.startsWith('/') ? searchParams.next : undefined
    ...
    <main className="mx-auto flex min-h-dvh max-w-[420px] flex-col justify-center gap-10 px-4">
      <div className="space-y-3">
        ...
        <p className="prose-readable text-sm text-muted-foreground">
          A sign-in link arrives in your inbox. No password required — but you can set one in
